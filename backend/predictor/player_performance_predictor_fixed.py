"""
Player Performance Predictor for Dream11 Team Predictor (Fixed Version)
...
class PlayerPerformancePredictor:
    ...
    def __init__(self, data_dir=None):
        ...
        self.batting_label_encoder = None
        self.bowling_label_encoder = None
        ...
        try:
            ...
            if y_classification is not None:
                ...
                for name, model in cls_models.items():
                    ...
                    # Use encoded labels
                    pipeline.fit(X_train, y_cls_train_encoded)
                    y_pred_encoded = pipeline.predict(X_test)
                    
                    # Convert back to string labels for evaluation
                ...
                y_pred_encoded = best_cls_model.predict(X_test)
                y_pred = label_encoder.inverse_transform(y_pred_encoded)
    ...
    def _train_bowling_models(self):
        ...
        try:
            ...
            if y_classification is not None:
                ...
                y_cls_train_encoded = label_encoder.fit_transform(y_cls_train)
                y_cls_test_encoded = label_encoder.transform(y_cls_test)
                self.bowling_label_encoder = label_encoder
    ...
    def _save_models(self, model_type):
        ...
        if model_type == 'batting':
            ...
            if self.batting_label_encoder is not None:
                joblib.dump(self.batting_label_encoder,
                           os.path.join(self.models_dir, f'batting_label_encoder_{timestamp}.pkl'))
            ...
            if self.bowling_label_encoder is not None:
                joblib.dump(self.bowling_label_encoder,
                           os.path.join(self.models_dir, f'bowling_label_encoder_{timestamp}.pkl'))
    ...
    def load_models(self, batting_reg_path=None, batting_cls_path=None, 
        ...
        try:
            ...
            # Also try to load preprocessors and label encoders
            preprocessor_files = [f for f in os.listdir(self.models_dir) if 'preprocessor' in f]
            encoder_files = [f for f in os.listdir(self.models_dir) if 'label_encoder' in f]
            ...
            for file in encoder_files:
                if file.startswith('batting_label_encoder_') and self.batting_label_encoder is None:
                    self.batting_label_encoder = joblib.load(os.path.join(self.models_dir, file))
                    logging.info(f"Loaded batting label encoder: {file}")
                
                if file.startswith('bowling_label_encoder_') and self.bowling_label_encoder is None:
                    self.bowling_label_encoder = joblib.load(os.path.join(self.models_dir, file))
                    logging.info(f"Loaded bowling label encoder: {file}")
        ...
        try:
            ...
            if prediction_type == 'batting':
                ...
                if performance_type in ['classification', 'both'] and self.batting_classification_model is not None:
                    ...
                    pred_encoded = self.batting_classification_model.predict(input_df)[0]
                    
                    # Convert back to class name
                    if self.batting_label_encoder is not None:
                        predicted_class = self.batting_label_encoder.inverse_transform([pred_encoded])[0]
                    else:
                        # Fallback if label encoder not available
                        predicted_class = 'Medium' if pred_encoded == 1 else ('High' if pred_encoded == 2 else 'Low')
                        
                    result['performance_class'] = predicted_class
                    
                    # Get class probabilities if available
                    try:
                        probs_encoded = self.batting_classification_model.predict_proba(input_df)[0]
                        if self.batting_label_encoder is not None:
                            classes = self.batting_label_encoder.classes_
                            result['class_probabilities'] = {
                                classes[i]: round(prob, 3) for i, prob in enumerate(probs_encoded)
                            }
                    except:
                        pass
            ...
            # Bowling prediction
                ...
                if performance_type in ['classification', 'both'] and self.bowling_classification_model is not None:
                    # Get encoded prediction
                    pred_encoded = self.bowling_classification_model.predict(input_df)[0]
                    
                    # Convert back to class name
                    if self.bowling_label_encoder is not None:
                        predicted_class = self.bowling_label_encoder.inverse_transform([pred_encoded])[0]
                    else:
                        # Fallback if label encoder not available
                        predicted_class = 'Medium' if pred_encoded == 1 else ('High' if pred_encoded == 2 else 'Low')
                    ...
                    # Get class probabilities if available
                    try:
                        probs_encoded = self.bowling_classification_model.predict_proba(input_df)[0]
                        if self.bowling_label_encoder is not None:
                            classes = self.bowling_label_encoder.classes_
                            result['class_probabilities'] = {
                                classes[i]: round(prob, 3) for i, prob in enumerate(probs_encoded)
                            }
                    except:
